            ...
            if (mutations != null) {
                ...
                for (Mutation m: mutations){
                    ...
                    for (int i = 0; i < m.mutations.length; i++) {
                        ...
                        if (path.length >1) {
                            path[path.length-2] = path[path.length-2] + String.format("_l%d_i%d_m%d", line, m.index, i);
                        }

                        FileOutputStream writer = new FileOutputStream(new File(file.getParent(), String.join(".", path)));
                        // write to file until current point
                        writer.write(content.toByteArray());

                        //write line with mutation
                        writer.write((str.substring(0,m.index) + mutation + str.substring(m.index + mutation.length())).getBytes());
